...
export function scheduleUpdateOnFiber(
  ...
  // 判断是否在ComponentWillUpdate 和 componentDidUpdate 中调用了setState
  checkForNestedUpdates();
  // 更新 root 以及 childExpirationTime ...
...
function ensureRootIsScheduled(root: FiberRoot) {
  ...
  // 如果正在render 的task 的 priority 和 expiration time 不正确。
  // 取消该任务并重新调度一个新的任务
  if (existingCallbackNode !== null) {
    ...
    // 执行 task.calllbase = null  将回调函数置空
  ...
  if (expirationTime === Sync) {
    ...
    callbackNode =  (performSyncWorkOnRoot.bind(null, root));
...
function performSyncWorkOnRoot(root) {
  ...
  if (lastExpiredTime !== NoWork) {
    ...
    if (
      ...
      // expired work.
      // 当前已经过期的，把当前渲染有效期置为 有效期 （后面会直接渲染）
      expirationTime = renderExpirationTime;
    } else {
      // Start a fresh tree.
      // 开启一个新的 tree
      // 如果渲染有效时间小于 节点最晚的有效期， 那就把 最晚时间赋给 expirationTime
...
function renderRootSync(root, expirationTime) {
  const prevExecutionContext = executionContext;
  executionContext |= RenderContext; // 合并上 renderContext
  const prevDispatcher = pushDispatcher(root);

  // If the root or expiration time have changed, throw out the existing stack
  // and prepare a fresh one. Otherwise we'll continue where we left off.
  // root 或者 expiration time 有改变，重置一个新的workInProgress
